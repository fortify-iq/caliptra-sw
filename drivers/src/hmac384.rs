...
use crate::{ Array4x12, CaliptraError, CaliptraResult, KeyReadArgs,
    KeyWriteArgs, Trng,
};
...
use core::usize;
use fortimac_hal::{FortimacErr, Fortimac384};
    ...
    pub fn hmac(
        ...
        match data {
            Hmac384Data::Slice(buf) => {
                hmac.digest(buf, &mut digest_bytes).map_err(|err| err.into_caliptra_err())?;
            },
            Hmac384Data::Key(key) => {
                key_vault_used = true;
                let kv = unsafe { KvReg::new() };
        ...
        match &mut tag {
            ...
            Hmac384Tag::Key(key) => {
                ...
                kv.regs_mut().key_ctrl().at(key.id.into()).write(|w| {w.dest_valid(valid)});
...
impl<'a> Hmac384Op<'a> {
    ...
    pub fn update(&mut self, data: &[u8]) -> CaliptraResult<()> {
        ...
        self.hmac_engine.update(data).map_err(|err| err.into_caliptra_err())?;
    ...
    pub fn finalize(mut self) -> CaliptraResult<()> {
        let mut digest_bytes = [0; 48];
        self.hmac_engine.finalize(&mut digest_bytes).map_err(|err| err.into_caliptra_err())?;
        let digest = Array4x12::from(digest_bytes);
